...
\item Maak het script\\
	...
	aan (kopieer dit uit een vorige editie en pas de editie aan). In dit script wordt de functie \verb+SVsurvey_create_codebook()+ aangeroepen. Deze functie laadt het bestand\\
		\url{01_code/03_process_editions/SVXXXX/input/SVXXXX_variables_survey.xlsx}\\
		in, koppelt dit via \verb+concept_id+ aan de juiste vraagverwoording, variable labels, codelijsten, en antwoordverwoordingen uit de Data Structuur Definitie, \path{Questions.xlsx} en \path{Answers.xlsx}, compileert het codeboek, en schrijft dit weg naar het bestand\\
		\path{03_deriveddata/04_Codebooks/Codebook_SVXXXX.xlsx}. Klaar!
\end{enumerate} 
...
\item De steekproefdata moet klaar staan in map\\
	\path{02_sourcedata/SVXXXX/01_sample}
...
\item De responsdata moet klaarstaan in map\\
	\path{02_sourcedata/SVXXXX/02_survey}\\
...
\item Maak de code\\
	\path{01_code\03_process_editions\SVXXXX\02_clean_data_SV0001.R}\\
	...
	\item Inladen en klaarzetten van de steekproefdata. Zorg hierbij ervoor dat
		\begin{itemize}
		\item de data nog enkel de variabelen bevatten die worden gebruikt voor de cleaning met de juiste naam (dus de variabelen opgelijst in \path{SVXXXX_variables_sample.xlsx}).
	...
	\item Inladen en klaarzetten van de surveydata. Zorg hierbij ervoor dat:
		...
		\item dat correctie op basis van open tekstvakken worden doorgevoerd. Hiervoor zet Tina het bestand\\ \path{01_code\03_process_editions\SVXXXX\input\SV0001_hercodering_obv_tekstvakken.xlsx}\\
		...
		\path{01_code\03_process_editions\SVXXXX\input\SV0014_hercodering_sport.xlsx}\\
		op basis van\\
		\path{01_code\03_process_editions\Metadata\Sports.xlsx}.\\
		Sporten 1 tot 15 worden bevraagd via selectievragen en moeten eventueel worden gecorigeerd op basis van de open antwoordvakken net zoals de andere variabelen in het bestand. Sporten 16-\ldots worden karakteriëel opgeslagen in variabele \verb+spt_othx+, als een semicolon-separated (;) lijst. Zorg hierbij er zeker voor dat de gestandardiseerde sportnamen uit \path{Sports.xlsx} worden gebruikt.
		\item de data nog enkel de variabelen bevatten die worden gebruikt voor de cleaning met de juiste naam (dus de variabelen opgelijst in 			\path{SVXXXX_variables_sample.xlsx}).
	...
	\path{03_deriveddata\05_Cleaned_data} als \path{.parquet}-bestanden. 
	\end{enumerate}
	...
	\item Een waarde in de surveydata komt niet voor in een codelijst. Dit kan zowel een fout zijn in het databestand als in de codelijst zelf, maar hier ook in bestand in het bestand \path{Answers.xlsx}. Je kan dezelfde oplossing kiezen als het punt hiervoor, of je kan bestand \path{Answers.xlsx} aanpassen. Pas \path{Answers.xlsx} echter enkel aan als het echt om een gevraagde kleine wijziging in vraagverwoording gaat, want deze wijzigingen zullen meegenomen worden naar de volgende edities. Als het veldwerkbureau gewoon een verkeerde waarde heeft aangeleverd, vervang je dit best manueel via functies zoals \verb+dplyr::replace_values()+ in een \verb+dplyr::mutate()+ stap.
	\item Variabelenamen zijn fout. Dan heb je opnieuw twee opties. Ofwel verander je de variabelenaam manueel in de code, ofwel corrigeer je de variabelenaam in de bestanden \path{SVXXXX_variables_sample.xlsx} of \path{SVXXXX_variables_survey.xlsx}. Die laatste optie is natuurlijk veel eleganter.
	\end{itemize}
\end{enumerate}
...
\path{01_code\03_process_editions\SVXXXX\03_create_VOS_SVXXXX.R}\\
aan. Deze roept de functie \verb+SVsurvey_create_VOS()+ aan, waarna een analysedataset wordt weggeschreven in\\
\path{03_deriveddata\06_Analysis_data}\\
en data met alle VOS'en in\\
\path{\\WV162699\fs_kb_dkb\svr\Beveiligde_data_VSA\999_Public_Use_Files\01_VOSdata\SVsurvey}
...
\path{\\WV162699\fs_kb_dkb\svr\Beveiligde_data_VSA\021_SVbevraging\03_deriveddata\06_Analysis_data}\\
Alle analysevariabelen worden ook opgelijst in\\
\path{01_code\03_process_editions\Metadata\Variables_for_VOS.xlsx}\\
...
\verb+type+ in \path{Variables_for_VOS.xlsx}. Dit zijn de VOS'en en worden weggeschreven in\\
\path{\\WV162699\fs_kb_dkb\svr\Beveiligde_data_VSA\999_Public_Use_Files\01_VOSdata\SVsurvey\VOS}.
...
\path{\\WV162699\fs_kb_dkb\svr\Beveiligde_data_VSA\999_Public_Use_Files\01_VOSdata\SVsurvey\respons_rates}
...
\item De mapping naar variabelen naar analysevariabelen kan gebeuren via de mappings in de codelijsten in \path{03_deriveddata\01_codelists}. Het is waarschijnlijk niet nuttig om een aparte dataset aan te maken.
...
\subsection{Creatie Public en Scientific Use Files}
...
Als de VOS-data klaar staan, kan hieruit een extractie gebeuren om datasets aan te maken voor de cijferpagina's. Dit gebeurt via het programma


Deze tabellen worden weggeschreven in de map\\
\path{\\WV162699\fs_kb_dkb\svr\Beveiligde_data_VSA\999_Public_Use_Files\02_CPdata}



\section{Nieuwe vragen}

Wat moet er gebeuren als er nieuwe vragen worden toegevoegd aan de SV-bevraging

\begin{enumerate}
\item codelijst aanmaken in het datamodel
\item concept toevoegen aan dsd
\item vraagverwoording toevoagen aan Questions.xlsx
\item antwoordverwoording toevoegen aan Answers.xlsx
\item variabele mee opnemen in cleaning, ze moet dus ook staan in het overzicht \path{SVXXXX_variables_survey.xlsx}
\item Als er speciale bewerkingen tijdens de cleaning moeten worden uitgevoerd, moet dit opgenomen worden in \path{SVsurvey_clean_data.R}
\item analyse van de variable moet worden toegevoegd aan de \path{SVsurvey_create_VOS.R}
\item Voeg de variabele toe aan bestand \path{} voor PUF en SUF
\item de aanmaak van een tabel voor DataWrapper moet worden toegevoegd 
\end{enumerate}
